...
class CalibrationForceAdmin(admin.ModelAdmin):
    list_display = ("cal_force_id", "apply_com", "apply_ten", "compress", "tension", "fullscale", "error", "update", "uuc_id")
    search_fields = ("cal_force_id",)
    exclude = ('uncer',)

@admin.register(CalibrationPressure)
class CalibrationPressureAdmin(admin.ModelAdmin):
    list_display = ("cal_pressure_id", "set", "m1", "m2", "m3", "m4", "avg", "error", "update", "uuc_id")
    search_fields = ("cal_pressure_id",)
    exclude = ('uncer',)

@admin.register(CalibrationTorque)
class CalibrationTorqueAdmin(admin.ModelAdmin):
    list_display = ("cal_torque_id", "cwset", "cw0", "cw90", "cw180", "cw270", "cw_avg", "cw_error", "ccwset", "ccw0", "ccw90", "ccw180", "ccw270", "ccw_avg", "ccw_error", "update", "uuc_id")
